...
def project(var, cut, weight, samplelist, pd, ntupledir):
    ...
    subdatachain = {}
    hist = {}
    binnedHist={} #declare dictionary of histograms for each samples.
    binnedHistData={} #declare dictionary of histograms for each samples data.
    tot=[]
    ##Express composite variable
    if var=="Zmass":
    ...
    for i, s in enumerate(samplelist):
        ...
        if "HIST" in cut: # Histogram written to file
            for j, ss in enumerate(samples[s]['files']):
                ...
                hist[s] = file[ss].Get("Events/" + histDir[var[0:2]] + "/" + var) if not s in hist else hist[s].Add(file[ss].Get("ntuple/" + histDir[var[0:2]] + "/" + var))
        else: # Project from tree
            chain[s] = TChain("Events")
            ##adding binned sample with respective weight
            for j, ss in enumerate(samples[s]['files']):
                if not 'data' in s:
                    print colr.HEADER +'subsample ----> ',ss , colr.ENDC
                    #chain[s].Add(ntupledir + ss + ".root")
                    subchain[ss] = TChain("Events")
                    subchain[ss].Add(ntupledir + ss + ".root")
                    if variable[var]['nbins']>0: binnedHist[ss] = TH1D(ss, ";"+variable[var]['title'], variable[var]['nbins'], variable[var]['min'], variable[var]['max']) # Init histogram
                    else: binnedHist[ss]=TH1D(ss,";"+variable[var]['title'], len(variable[var]['bins'])-1, array('f', variable[var]['bins']))
                    binnedHist[ss].Sumw2()
                    #print "xsec= ", tag[ss]['xsec']," ; nevents= ", tag[ss]['nevents']," ; kfactor= ", tag[ss]['kfactor']," ; matcheff= ", tag[ss]['matcheff'], " ; EventsFromTree", subchain[ss].GetEntries()
                    #nevents = subchain[ss].GetEntries()
                    nevents = tag[ss]['nevents']
                    xs = tag[ss]['xsec']
                    LumiMC = nevents/xs
                    Weight = float(weight) / float(LumiMC)
                    print colr.OKBLUE +"number of event =", nevents , colr.ENDC
                    print colr.OKBLUE +"cross section =", xs , colr.ENDC
                    print colr.OKBLUE +"integrated luminosity (/pb)-->", float(weight) , colr.ENDC
                    print colr.OKGREEN +"manual apply weight-->", Weight , colr.ENDC
                    #print "Drawing subsample one by one"
                    #WGT="ZPtCorr"
                    #subchain[ss].Project(ss, "%s" %VAR , "(%s*%s)*(%s)" %(Weight,WGT,cut))
                    subchain[ss].Project(ss, "%s" %VAR , "(%s)*(%s)" %(Weight,cut))
                    #subchain[ss].Draw("%s>>%s" %(var,ss) , "%s*( (HLT_IsoMu22 || HLT_IsoMu22_eta2p1 || HLT_IsoMu24 || HLT_Mu45_eta2p1 || HLT_Mu50) && %s)" %(Weight,cut))
                   
                elif ('data' in s and ss in pd):
                    print colr.WARNING +'subdata ----> ',ss , colr.ENDC
                    #chain[s].Add(ntupledir + ss + ".root")
                    subdatachain[ss] = TChain("Events")
                    subdatachain[ss].Add(ntupledir + ss + ".root")
                    if variable[var]['nbins']>0: binnedHistData[ss] = TH1D(ss, ";"+variable[var]['title'], variable[var]['nbins'], variable[var]['min'], variable[var]['max']) # Init histogram
                    else: binnedHistData[ss]=TH1D(ss,";"+variable[var]['title'], len(variable[var]['bins'])-1, array('f', variable[var]['bins']))
                    binnedHistData[ss].Sumw2()
                    #Remove HLT requirement
                    subcut= cut.replace(muonTrigger,"1==1")
                    #subcut=cut
                    print "NO HLT REQUIREMENT FOR DATA"
                    print subcut
                    subdatachain[ss].Project(ss, "%s" %VAR , "%s" %subcut)
                    #subdatachain[ss].Draw("%s>>%s" %(var,ss) , "%s" %cut)
                    print colr.OKBLUE + 'sub-Data histogram entries --> ', binnedHistData[ss].GetEntries(), colr.ENDC
                    print colr.OKBLUE + 'sub-Data histogram integral --> ', binnedHistData[ss].Integral(), colr.ENDC
                    print colr.UNDERLINE + 'sub-Data histogram content --> ', binnedHistData[ss].Print(), colr.ENDC

            #final histogram to be saved.
            ...
            if not 'data' in s:
                iters=0
                for key in sorted(binnedHist.iterkeys()):
                    iters+=1
                    print colr.OKBLUE + 'sub-MC histogram entries --> ', binnedHist[key].GetEntries(), colr.ENDC
                    print colr.OKBLUE + 'sub-MC histogram integral --> ', binnedHist[key].Integral(), colr.ENDC
                    print colr.UNDERLINE + 'sub-MC histogram content --> ', binnedHist[key].Print(), colr.ENDC
                    if iters ==1:
                        hist[s]=binnedHist[key].Clone(s)
                    else:
                        hist[s].Add(binnedHist[key])
                        #hist[s].SetOption("%s" %(sum(tot)))
                    print colr.BOLD + 'MC histogram entries --> ', hist[s].GetEntries(), colr.ENDC
                    print colr.BOLD + 'MC histogram integral --> ', hist[s].Integral(), colr.ENDC
                    print colr.UNDERLINE + 'MC histogram content --> ', hist[s].Print(), colr.ENDC
            elif 'data' in s: 
                print colr.HEADER + 'Drawing data...', colr.ENDC
                #chain[s].Project(s, var, cut)
                #chain[s].Draw("%s>>%s" %(var,s), "%s" %cut)
                iters=0
                for key in sorted(binnedHistData.iterkeys()):
                    iters+=1
                    print colr.OKBLUE + 'sub-DATA histogram entries --> ', binnedHistData[key].GetEntries(), colr.ENDC
                    print colr.OKBLUE + 'sub-DATA histogram integral --> ', binnedHistData[key].Integral(), colr.ENDC
                    print colr.UNDERLINE + 'sub-DATA histogram content --> ', binnedHistData[key].Print(), colr.ENDC
                    if iters ==1:
                        hist[s]=binnedHistData[key].Clone(s)
                    else:
                        hist[s].Add(binnedHistData[key])
                    print colr.BOLD + 'DATA histogram entries --> ', hist[s].GetEntries(), colr.ENDC
                    print colr.BOLD + 'DATA histogram integral --> ', hist[s].Integral(), colr.ENDC
		    print colr.UNDERLINE + 'DATA histogram content --> ', hist[s].Print(), colr.ENDC


                
                entries=hist[s].GetEntries()
                integral=hist[s].Integral()
                ##print colr.OKBLUE + 'Data histogram entries --> ', hist[s].GetEntries(), colr.ENDC
                ##print colr.OKBLUE + 'Data histogram integral --> ', hist[s].Integral(), colr.ENDC
                #hist[s].Scale(float(entries/integral))
                #hist[s].SetOption("%s" %chain[s].GetEntriesFast())
                #print colr.UNDERLINE + 'Data histogram content --> ', hist[s].Print(), colr.ENDC
...
def printTable(hist, sign=[]):
    ...
    for i, s in enumerate(sign):
        ...
        print "%-20s" % s, "\t%-10.2f" % hist[s].Integral(), "\t%-10.0f" % (hist[s].GetEntries()-2), "\t%-10.2f" % (100.*hist[s].GetEntries()) if float(hist[s].GetOption()) > 0 else 0, "%"
    print "-"*80
